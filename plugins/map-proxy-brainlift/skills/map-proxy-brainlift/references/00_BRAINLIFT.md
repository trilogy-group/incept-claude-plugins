...
Structural caveat (grade-range edges). The simulated reliable range starts at RIT 181, but the national G2 fall mean is ~170 — the median G2 fall student sits below the floor. Which tail binds is grade-specific, per the re-baselined population data [EXTERNAL-REVIEW + COMPUTED — see the population-baseline analysis in the engine repository]: **G2 floor, G3 floor, G4 mild ceiling, G5 ceiling** (~20% of current G5 sit above even RIT 235). Both edges are named, flagged in score reports, and addressed by the merged tail expansions (floor to ~RIT 155, ceiling to 245 with the G5 super-ceiling set — harness-loadable, adoption pending calibration); neither is hidden inside an average.
...
The v3 parity run (2026-07-03) cleared what the v1 production run had flagged: RMSE 3.44 RIT (was 4.73 — within the <4.0 target), bias −0.38 RIT (the +2.62 overestimate was root-caused to the Owen running estimate's prior shrinkage and resolved by re-basing the final score on the shipped MLEF value). The one remaining reported gap — reliability 0.929 vs the ≥0.93 target (≈0.001) — plus the residual −0.4 to −1.0 RIT negative bias in the 175–190 bands share one root cause family: expert-assigned b-parameters under the wide G2–G5 prior. The robustness simulation shows the *algorithm* tolerates parameter uncertainty; empirical calibration — accrued through regular use toward a ~300-student-equivalent response volume for the core band — replaces the guessed parameters. The plan is designed to resolve this; simulation supports but cannot guarantee it — which is exactly why the ask is real usage (a ~40-student feasibility pilot, then calibration accruing from regular sessions), not more simulation.
...
Pool ratio stated both ways: 650/40 = 16.3× session length; against Stocking's recommended 10× floor (400 items), 1.6× — adequate, not lavish, and the staged expansions raise it further (766 harness-loadable). The binding constraints are the edges, and the re-baselined population data says which edge binds where [EXTERNAL-REVIEW + COMPUTED]: **G2 floor** (the only grade with a visible below-155 tail), **G3 floor** (decisively on the true 46th-percentile population), **G4 mild ceiling**, **G5 ceiling — badly** (~20% of current G5 above even RIT 235). The old ceiling-first framing keyed to "G3 mean 206.4" is retired with that number. Bank expansion priorities follow the population, not the average — hence the merged floor extension to ~RIT 155, the G5 super-ceiling set at RIT 231–250, and explicit routing of sub-155 students to the official K–2 MAP.
...
### Summary 4 — The engine passes its v3 simulation suite, with one disclosed gap

- v3 run (2026-07-03): production selection path + randomesque selection, 650 items, G2–G5 prior N(−1.0, 1.2), TimeBack-parity configuration; ~17,900 sessions across 7 studies (the v3 validation report in the engine repository).
- Ability recovery r = 0.963 (≥0.90 benchmark); RMSE 3.44 RIT (<4.0); bias −0.38 RIT (reliable-range −0.27; max per-band 0.83); SEM 3.18 RIT (≤4.0); blueprint 100% (±1 vs 14/13/13); TEI floor 100% (mean 16.2 ≥ 12); 0 MLEF divergences; robust to parameter error to σ=1.0 logits.
- The one reported gap: marginal reliability 0.929 vs the ≥0.93 target (≈0.001 short; ≪0.96 published) — empirical calibration is the closer.
- History reported, not smoothed: the v1 production re-run flagged RMSE 4.73 and bias +2.62; v2 root-caused the bias to the Owen running estimate's prior shrinkage and re-based the final score on the shipped MLEF value; v3 re-validates under the parity constants. Exposure follow-ups remain: 38/650 over-exposed, 241/650 never used, two easy-band cells starved.
- Implication: algorithm validity is demonstrated on the production path; residual precision awaits calibration. *(Simulation.)*
...
- b-parameters expert-assigned; SE(b̂) effectively infinite pre-calibration (Wright & Stone 1979). Score reports meanwhile emit a b-uncertainty-adjusted SE (Fisher SE × 1.08 under v3, `se_basis`-labelled) rather than pretending calibrated precision.
- The internal cross-bank consistency simulation (synthetic — explicitly NOT MAP agreement) used a synthetic MAP-like bank: r 0.912, 95% LoA [−10.7, +10.2] RIT under v3 — a consistency check, not evidence of equivalence.
...
- Per-grade tails (bank ~RIT 155–245): G2 floor binds; G3 floor binds; G4 mild ceiling; G5 ceiling binds badly (~20% of current G5 above RIT 235). *(Computed + External-review.)*
...
- Bank: 650 operational items across 13 files (counted programmatically); b_provisional ≈ RIT 175–220; formats 265/102/90/76/63/54 (MCQ/seq/MSQ/gap/EBSR/hot-text). Staged: +92 edge (PR #30) and +44 standard-gap/super-ceiling (PR #35) = 766 harness-loadable, b −4.45→+4.55 ≈ RIT 155–245, adoption pending calibration. *(Engine fixtures.)*
- Simulations (v3 run 2026-07-03: production path + randomesque, 650 items, G2–G5 prior, TimeBack-parity configuration): r=0.963; RMSE 3.44; bias −0.38 RIT; reliability 0.929; SEM 3.18; blueprint 100% (±1 vs 14/13/13); TEI floor 100% (mean 16.2); 0 divergences in ~17,900 sessions; reliable range RIT 181–227; robustness to σ=1.0. *(The v3 validation report in the engine repository — authoritative.)*
