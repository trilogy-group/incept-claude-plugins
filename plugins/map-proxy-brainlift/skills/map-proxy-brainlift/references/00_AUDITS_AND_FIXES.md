...
| :---- | :---- | :---- |
| 1 | Summary tables reported reliability as "0.930 ✅" against a ≥0.93 benchmark the underlying reports state as 0.928, below | Reliability is stated honestly everywhere — **0.929 under the v3 parity run, still 0.001 short of the benchmark** — with the context that MAP's published Reading 2–5 figure is 0.96 |
| 2 | The one failing metric (RMSE 4.12 vs <4.0 target) was omitted from every summary table | RMSE appears in every summary table — the v1 production re-run regressed it to 4.73 and we reported that; the v3 parity run measures **3.44 RIT, now within the <4.0 target** (Case for Approval §7) |
...
| 4 | Simulations were G3-scoped (630 items, G3 prior, simplified selector) but described as "G2–G5, production code" | **Re-run on the true production path (v1 2026-07-02), root-caused and re-based (v2), and re-validated under the TimeBack-parity configuration (v3 2026-07-03)** — all 650 items, G2–G5 prior; the Case for Approval §7 carries the v3 numbers, and the v1 regression story is preserved, not overwritten |
| 5 | "Reliable across full G2–G5" — but the reliable floor (RIT 181) sits above the national G2 fall mean (~170) | Range re-scoped honestly: G3–G5 plus above-average G2; G2 band expansion queued; G2 reports carry a floor flag |
...
| 2 | Reported SE understates true uncertainty under provisional b values | **Shipped** — score reports now emit b-uncertainty-adjusted SE (×1.08 under v3) with an `se_basis` field; auto-removed post-calibration |
...
| 7 | G2 low tail effectively outside the instrument | **Owned** — floor expansion to ~RIT 155 merged (+92 edge items, PR #30); sub-155 students route to official K–2 MAP, stated explicitly |
